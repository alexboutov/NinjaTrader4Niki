#!/usr/bin/env python3
"""
AnalyzeMarketReplaySessionLocal.py
Analyzes NinjaTrader Market Replay sessions from local laptop logs.
Generates {Mon}{DD}_MR_{Start}_{End}_Trading_Analysis{N}.txt report.

Usage: 
    python AnalyzeMarketReplaySessionLocal.py <start_date> <end_date> [--trader-log <path>] [--csv-log <path>]

Examples:
    python AnalyzeMarketReplaySessionLocal.py 2025-12-19 2025-12-31
    python AnalyzeMarketReplaySessionLocal.py 2025-12-19 2025-12-31 --trader-log "path/to/ActiveNikiTrader_*.txt"
"""
...
# === CONFIGURATION ===
LOCAL_LOG_PATH = r"C:\Users\alexb\OneDrive\Documents\NinjaTrader 8\log"
OUTPUT_BASE_PATH = r"C:\Users\alexb\Downloads\ActiveNiki"
...
def generate_report(config, signals, roundtrips, bars, daily_events, start_date, end_date):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append(f"MARKET REPLAY SESSION ANALYSIS (LOCAL)")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python AnalyzeMarketReplaySessionLocal.py <start_date> <end_date> [--trader-log <path>] [--csv-log <path>]")
        print("Example: python AnalyzeMarketReplaySessionLocal.py 2025-12-19 2025-12-31")
        sys.exit(1)
    ...
    if trader_log_path is None:
        trader_log_path = find_latest_file("ActiveNikiTrader_*.txt", LOCAL_LOG_PATH)
    if csv_log_path is None:
        csv_log_path = find_latest_file("IndicatorValues_*.csv", LOCAL_LOG_PATH)
    
    if not trader_log_path or not os.path.exists(trader_log_path):
        print(f"ERROR: ActiveNikiTrader log not found in {LOCAL_LOG_PATH}")
        sys.exit(1)
    
    print(f"Market Replay Analysis (LOCAL)")
