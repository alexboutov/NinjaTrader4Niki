...
def parse_trades(filepath):
    ...
    for line in content.splitlines():
        ...
        trades.append({
            'timestamp': timestamp,
            'datetime_str': f"{date_str} {time_str}",  # CHANGED: Full date-time string
            'time_str': time_str,
            'action': action,
            'direction': direction,
            'price': fill_price,
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            try:
                ...
                bars.append({
                    'timestamp': timestamp,
                    'datetime_str': timestamp.strftime('%Y-%m-%d %H:%M:%S'),  # CHANGED: Full date-time string
        ...
        if signal_match:
            ...
            for j in range(1, 20):
                if i + j < len(lines):
                    ...
                    if 'ΓòÜ' in next_line or '├óΓÇó┼í' in next_line:
            ...
            signals.append({
                'source': source,
                'time_str': time_str,
                'datetime_str': f"{signal_date} {time_str}",  # CHANGED: Full date-time string
...
def parse_trader_orders_and_closes(filepath, date_str):
    ...
    TRADE CLOSED format:
        Γ£à TRADE CLOSED: P&L $600.00 | Daily P&L: $600.00 (1 trades)
        Γ¥î TRADE CLOSED: P&L $-185.00 | Daily P&L: $415.00 (2 trades)
    ...
    for i, line in enumerate(lines):
        ...
        if order_match:
            ...
            if current_signal_time and current_signal_date:
                orders.append({
                    ...
                    'datetime_str': f"{current_signal_date} {current_signal_time}",  # CHANGED: Full date-time string
        ...
        if entry_fill_match:
            ...
            if orders and orders[-1]['direction'] == direction:
                ...
                orders[-1]['datetime_str'] = f"{trade_date} {trade_time}"  # CHANGED: Full date-time string
        
        # Parse TRADE CLOSED - NEW FORMAT with direction, entry, exit, reason, and optional exit slippage
        # Format: Γ£à TRADE CLOSED: SHORT | Entry=25187.00 Exit=25165.00 | +88t $434.84 | Reason: TRAIL | Exit Slip: +4t
        ...
        if closed_match:
            ...
            closes.append({
                ...
                'datetime_str': f"{close_date} {time_str}",  # CHANGED: Full date-time string
                'time_str': time_str,
                'direction': direction,
...
def parse_trader_closed_trades(filepath, date_str):
    ...
    NEW Format: Γ£à TRADE CLOSED: SHORT | Entry=25187.00 Exit=25165.00 | +88t $434.84 | Reason: TRAIL
    OLD Format: Γ¥î TRADE CLOSED: P&L $-340.00 | Daily P&L: $-340.00 (1 trades)
    """
    closed_trades = []
    if not os.path.exists(filepath):
